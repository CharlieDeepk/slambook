<?php
    ...
    if(array_key_exists("id", $_SESSION)) {
        ...
        if(mysqli_connect_error()) {
            ...
            $query = "SELECT `pgno` FROM `dairy_content` WHERE `sender_id` = '".$_SESSION['id']."' LIMIT 1";
            $result = mysqli_query($link, $query);
            
            if(mysqli_num_rows($result) > 0) {
                
                $query = "SELECT * FROM `dairy_content` WHERE `sender_id` = '".$_SESSION['id']."'";
                $result = mysqli_query($link, $query);
                ...
                $tableSorry = '<div style="height: 500px;"><p class="lead">Sorry you haven\'t sent slambook page to any one.</p></div>';    
            }
